...
- **DMX** 用 Watcom C 的 32 位元編譯器（makefile 裡是 `wcc386p`，`flat` model）。它的 makefile
  ...
  音效卡的中斷照樣會來，需要一段 16 位元碼在真實模式下接住它（強推論）。
  版本之間的差異與建置變體見 [DMX 的五份封存](../30-dmx/version-history.md)。
...
數學函式庫與 Windows 版函式庫大部分相同，少數浮點相關模組用手上的工具鏈與參數組合都重現不出來，這幾個模組只以原始碼為證據。Borland C++ 2.0 另有一個 1991-08 的改版
（Application Frameworks 套裝），它的 C 函式庫只有 `SCROLL` 一個模組不同（程式碼變長，多參照
`puttext`／`gettext`）：`directvideo` 開著、視窗捲動一列時，4 月版空出來的那一列留著舊字，8 月版清成空白，兩版連結同一支測試程式實跑確認過。所以讀這份原始碼得到的結論，可以直接套到 BC++ 2.0 編出的程式上；
...
| 4 月版捲動一列時空出的列留著舊字、8 月版清空 | 同一支 conio 測試程式（small 模型）分別連結兩版 `CS.LIB`，在 dosgolem 執行後讀回畫面 | 已證實（實跑） |
...
- Borland 數學函式庫有六個模組（`STRTOD`、`FPERR`、`_MATHERR`、`SCANTOD`、`EFCVT`、`GCVT`）、Windows 版函式庫有兩個組語模組
  （`E87TRANS`、`FPINIT`）重編後與出貨版不同。`EFCVT`、`GCVT` 關掉暫存器變數就對得上，但全庫都關又會讓其他模組對不上；其餘四個用 BCC 2.0 的各種參數與 Turbo C 2.0 都重現不出來，出貨目的碼是用哪一版工具或原始碼建出來的仍未知；
  Microsoft 的 CRT 還沒有編譯器可以對拍。
- DSMI 在保護模式下的支援程度；DMX 在各音效卡上的中斷處理細節。
- DMX 這份封存的流出管道。權利人本身沒有疑義：每支原始碼的版權頁都寫 Digital Expressions, Inc.，
  並標示機密與專有；公開資料也一致指出 id Software 因為不擁有這套庫，1997 年釋出 Doom 原始碼時
  必須先拿掉音效系統。查不到任何釋出或授權聲明，本專案因此只做原始碼層級的閱讀。
