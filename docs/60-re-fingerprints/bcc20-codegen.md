...
`-O`（跳躍最佳化）、`-G`（速度優先）、`-1`（80186 指令）、`-r-`（不用暫存器變數）都會換掉部分樣板。
...
BCC 替每個函式建立標準堆疊框：`55 8B EC`（`push bp; mov bp,sp`）。接著依序是：
...
`-k`（標準堆疊框）在 BCC 2.0 其實是預設開啟：加 `-k` 與不加，目的檔完全相同。
...
| pascal 函式 | 符號名轉大寫；參數由左到右推入，所以第一個參數在較高位移；被呼叫端 `C2 nn 00` 清堆疊 | |
...
值表搜尋是一個小迴圈：CX 放 case 數、BX 指到表頭，逐個 word 比對，`43 43`（BX 加 2）、`E2 F4`（`loop`）；
...
五個模型逐函式比較 BCC 的組語輸出，34 個函式裡 29 個只分成三種：small＝compact、medium＝large、huge。
差別只在 near／far 與 huge 的 DS 載入。會分成五種的，是用到資料指標的函式（指標參數、回傳指標、`main`）。
...
| 運算 | 暫存器 | AX | 記憶體 |
|---|---|---|---|
| `and` 12 | — | `25 0C 00` | `81 26 ?? ?? 0C 00` |
| `and` 3 | `81 E3 03 00` | | |
| `or` 3 | `81 CE 03 00` | | |
| `xor` 5 | `81 F6 05 00` | | |
| `add` 7 | `83 C6 07` | | `83 06 ?? ?? 07` |
| `cmp` 9 | `83 FE 09` | | |
| `sub` 2 | `4E 4E`（`dec si` 兩次） | | |
...
並在 `9B` 的位置掛一個外部符號的修正（`FIDRQQ` 等，目的檔的 `EXTDEF` 看得到 `FIWRQQ`、`FIDRQQ`）。

- **連結模擬器函式庫（預設）**：`FIDRQQ` 的值是 `5C32h`。它加到 `9B D8`–`9B DF` 這個 word 上，正好得到 `CD 34`–`CD 3B`，
...
`-f87` 不改變目的檔，五個模型比對都相同。浮點轉 `long` 前還有一組 `90 9B`（`nop; fwait`），掛 `FIWRQQ`（`A23Dh`），
...
對 BC++ 2.0 附的四支工具，直接在檔案位元組裡數上面幾個樣式（沒有對齊指令邊界，會有少量偶然命中）：
...
| `5F 5E 5D CB`／`5F 5E 5D C3` | 15／0 | 0／0 | 0／0 | 0／15 |
...
| `-k`、`-Z`、`-f87` 不改變目的檔 | 同上，逐記錄比對 OMF 語意記錄 | 已證實（實測） |
