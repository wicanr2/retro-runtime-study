...
（CP/M 時代用來標記檔案結束，因為當時的檔案大小以 128 bytes 為單位，不精確到位元組）。
...
|---|---|---|
| > 0 | 緩衝區裡還有這麼多位元組可讀 | `getc` 先減一，成功就直接取 `*curp++` |
| < 0 | 緩衝區還能放 `−level − 1` 個位元組 | `putc` 先加一，還沒滿就直接寫 `*curp++` |
...
| 串流 | 決定的地方 | 結果 |
|---|---|---|
| `fopen` 開的檔案 | `fopen` | 終端機（`isatty`）→ 行緩衝；其餘 → 全緩衝，512 bytes |
...
3. **文字模式再掃一次緩衝區**，每看到一個 `\n` 就加 1——因為那個 `\n` 在檔案裡占兩個位元組。
4. 寫入方向把這個補償值加上去，讀取方向減掉。

`fseek` 的 `SEEK_CUR` 也先減掉同一個補償值，再把剩下的交給 `lseek`；然後清掉方向與檔尾旗標、把緩衝區歸零。
...
| 2、3 | `ENOENT` | 檔案或路徑找不到 |
| 4 | `EMFILE` | 開太多檔 |
| 5 | `EACCES` | 存取被拒（唯讀、目錄、共享衝突） |
| 6 | `EBADF` | 代號不對 |
| 8 | `ENOMEM` | 記憶體不足 |
| 19 | `EROFS` | 寫入唯讀媒體 |
| 80 | `EEXIST` | 檔案已存在（`O_EXCL`） |
...
| `ungetc` 一個字元 | `ftell` 少 1；再讀一次拿回同一個字元 |
...
- `BUFSIZ` ＝ 512、`FOPEN_MAX` ＝ 20、代號表 20 格。
- `dup` 出來的代號與原代號共用檔案位置。
- `_fmode` 預設是文字模式。
