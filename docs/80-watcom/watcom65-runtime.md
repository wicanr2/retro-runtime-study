...
| large、huge | **每個模組自己一個段**，名字是「模組名＋`_TEXT`」（`strlen_TEXT`、`exit_TEXT`⋯⋯，各 221 個） |
...
| 程式結束 | 回傳碼交給 DOS 的結束服務；啟動階段的錯誤訊息直接寫到標準輸出，不經過 `stdio` |
...
十個 helper 模組的名稱、大小與語意、三個模型的程式碼段命名規律與那 28 個共用 `_TEXT` 的模組名單。方法是自寫的 OMF（Object Module Format，DOS 時代目的檔與函式庫的格式）解析器，
