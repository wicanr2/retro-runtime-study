...
triggers:
  ...
  - 看到 `CLIBM_TEXT`、`CLIBL_TEXT` 這種段名，想知道它代表什麼
  - 想知道 Watcom 的暫存器呼叫慣例是從哪一版開始的
  - 遇到 `__PTS`、`__PIA`、`__STK` 這種符號，想知道編譯器替程式偷偷呼叫了什麼
  - 要做 remake，想對照 Borland 與 Watcom 的啟動碼做了哪些事
symbols: [__STK, __STKOVERFLOW_, __PTS, __PTC, __PIA, __PIS, __PCA, __PCS, __I4M, __U4M, __I4D, __U4D, __I4FS, __U4FS, __ModF, __IsTable, __CMain, __psp, __osmajor, __osminor, __STACKTOP, __STACKLOW, __curbrk, __dynend, __8087, __stacksize, _cstart_, _chk8087_, CLIBM_TEXT, CLIBS, CLIBM, CLIBL, CLIBH, MATH87]
related: [borland-crtl/memory-model-macros, borland-crtl/startup-and-exit, borland-crtl/compiler-helpers, dmx/version-history]
---
...
另外 98 個是前置底線的資料與 helper。反組譯證實參數依序放 AX、DX——
`strlen` 從 AX 拿指標，`strcpy` 從 AX 拿目的、DX 拿來源。

這條慣例活得很久。1989 年的 Watcom C/386 7.0 起，同一套函式庫出兩份，
檔名用 `r` 與 `s` 區分暫存器版與堆疊版；到了 1993 年，DOOM 那批遊戲用的音效函式庫 DMX
...
比例大約是 267 比 98（medium model 的 C 函式庫）。
...
| 符號 | 做什麼 |
|---|---|
| `__I4M`／`__U4M` | 32 位元乘法，有號與無號 |
| `__I4D`／`__U4D` | 32 位元除法 |
| `__I4FS`／`__U4FS`、`__ModF` | 整數與浮點之間的轉換、取模 |
| `__PTS` | 兩個 far 指標**相減** |
| `__PTC` | 兩個 far 指標**比較** |
...
`__PTC` 還多一個捷徑——段相同的話直接比位移就好，省下正規化。

`__PIA` 這一組處理的是另一種情況：huge 指標加上一個整數之後，位移可能超過 16 bytes 的範圍，
於是要把超出的部分折回段裡，讓位移永遠落在 0 到 15。**這正是 huge 模型「陣列可以大於 64 KB」的代價**：
每一次指標算術都變成一次函式呼叫。

`__PIA` 與 `__PCA` 的差別只在指標放在哪一組暫存器——編譯器依當下哪個暫存器有空來挑，
省掉一次搬移。這種「同一個運算給好幾個入口」的做法，在 Borland 的 helper 裡也看得到
...
1. **模型定義成位元旗標。** 「程式碼是遠的」「資料是遠的」「huge」「tiny」各佔一個位元
   （1、2、4、8），六個模型就是這些位元的組合：small 是兩個零、large 是遠程式碼加遠資料、
   compact 是近程式碼加遠資料，依此類推。
...
出貨的函式庫也照這個切法分份：五個模型各一個 C 函式庫，tiny 沒有自己的庫、
只有一個獨立的啟動目的檔。這一點和 Borland 完全一樣。

浮點則不同：Watcom 分成軟體版與 8087 版**兩套獨立的函式庫**，各五個模型；
...
4. 設好四個全域變數：堆疊頂、堆疊下限、目前的記憶體上緣、以及堆積可以長到哪裡。
...
8. 偵測有沒有 8087。
9. 跳進 C 那一側的進入點，由它處理命令列切割、開啟標準檔案，最後呼叫 `main`。

結束時走 `AH=4Ch` 把回傳碼交給 DOS；啟動階段的錯誤訊息用 `AH=40h` 直接寫到標準輸出。
...
差別在兩處：Watcom 的啟動碼**沒有 Borland 那種初始化表與結束表**
（`atexit` 是獨立的模組，不是啟動碼的一部分），也**沒有空指標檢查**——
Borland 會在結束時比對資料段開頭那幾個 bytes 有沒有被寫壞，Watcom 這一版不做這件事。
...
| `CLIBM_TEXT`、`CLIBL_TEXT` 這類段名 | 函式庫的程式碼段用「庫名＋`_TEXT`」，**段名直接說出它連了哪個模型的庫**。啟動碼本身用 `_TEXT` |
| `__psp`、`__osmajor`、`__STACKTOP`、`__curbrk` | 啟動碼設定的全域變數，一組出現 |
| `__STK`、`__PTS`、`__PIA` 這些 helper | 前置兩個底線、模組極小（一百多 bytes）、沒有外部參考 |
...
`strlen` 從 AX 取參數、`strcpy` 從 AX 與 DX 取參數、十個 helper 模組的名稱、大小與語意、
函式庫的段名。方法是自寫的 OMF 解析器加上**原廠自己的反組譯器**跑在無頭 DOS 執行器裡。

**已證實（原文）**：可逐函式指定傳參暫存器的 pragma、記憶體模型的位元旗標與三層巨集機制、
啟動碼的流程與它呼叫的 DOS 服務、函式庫的模型分份方式。來源是原廠說明檔與原廠附的啟動碼原始碼。

**強推論**：「尾端底線＝暫存器慣例」這個對應。6.5 的說明檔沒有明說預設慣例是哪一種，
