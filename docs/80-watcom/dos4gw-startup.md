...
9.01 的出貨函式庫（`clib3r`／`clib3s`，Easy OMF-386 變體：外層封裝照標準 OMF library 走、
模組記錄的欄位加寬，細節見 [總表那篇](watcom-lineage.md)）也已逐符號解析：
各 334 個模組、563 個定義的公開符號，每筆記錄通過 OMF 校驗和，
並與 7.0 原廠反組譯器（16 位元版）在三個模組上對照過符號與段——
`__STK`、`__I4FS`／`__U4FS`（`ifs` 模組）、`__ModF`（`amodf`）、
`__IsTable`／`_IsTable`（`istable`；堆疊版的資料類符號少一個底線）都是定義，
`__PTS`／`__PIA` 那組 16 位元 helper 沒有定義也沒有參照。
`stk` 模組另新增 `__CHK`／`__GRO`：反組譯顯示 `__STK` 檢查通過後呼叫 `__GRO`，
後者以 4K 步階從 ESP 向下逐頁寫入；檢查不過則走 `__exit_with_msg_` 結束。
...
- **Watcom 的 `execvp` 在 DOS 上到底是覆蓋還是等待子程序結束**：`process.h` 有 `P_OVERLAY`
  這個模式旗標；實作在 C 函式庫裡，現在解得開符號層，但這一塊還沒反組譯。
...
- 9.5 與 8.0 沒有下載。
- 沒有實跑：9.x 的工具是 32 位元保護模式程式，dosgolem（本系列用的無頭 DOS 執行器）目前跑不動。
