...
- **第四批（2026-09-18 完成）**：**版本演進總表**（`docs/80-watcom/watcom-lineage.md`，
  ...
  這是 M4 目標本身的驗收項。
- **第五批（本輪）**：只做 **WPK 解碼器**——它是唯一擋住 9.01 深度的卡點，
  解開就一次補上 9.01 的內容缺口，而且 8.0 與 9.5 是同一個封裝格式，會一起解鎖。
  **結果：沒解開，但卡點從「整個解碼器都不對」收斂成單一一點**，見下。
- **後續批次（本輪不做）**：8.0 的差異、LE／LX 格式細節。

### 第五批的結果（WPK 解碼器，未完成）

換了路線：不再用 Python 手工移植，改成把 open-watcom 的解碼核心切出來、配自己寫的 I/O
用 gcc 編成獨立解包器。這個決定一次排除了「是不是我移植錯」，並暴露兩個先前看不到的差異。

**修掉的兩個差異**

1. open-watcom 的 `wqsort.c` 裡，區塊交換那段假設 `long` 是 4 bytes；在 64 位元 Linux 上
   `long` 是 8，它一次交換 8 bytes 卻只前進 4，越界覆寫相鄰元素。症狀是排序失序或 segfault，
   **而且不會報錯**。先前手工移植失敗的根因就在這裡——照抄了一個在現代環境不成立的實作，
   於是「移植得夠不夠忠實」這個問題從頭到尾問錯方向。
2. 碼表條目數，1992 版不加 1（open-watcom 版會多讀一個位元組，位元流整個偏移）。
   這是手算封包的位元流起點推出來的，改掉之後第一個符號解出 `/`，與 C 標頭的開頭相符。

**已經確認正確的**：資料起點、碼表格式、碼長分配（最短碼給空格、最長碼給罕見符號）、
canonical 方向、碼值空間正好填滿、位元流狀態機（原廠版與自己重寫的直觀版互為正對照）、
LZSS 視窗與 match（解出的內容有重複片段）。

**剩下的單一卡點**：同一碼長內，符號按什麼順序對應碼值。它由當年那個 qsort 對**等值元素**
的處理順序決定，壓縮端與解壓端當年共用同一個函式。試過三種都不對（修好的 `wqsort`、
穩定遞增、群組內反轉）。`wqsort.c` 自己的註解說它是後來才為了跨平台固定下來的，
未必等於 1992 年 CLIB 裡那一個。

**順帶排除的一條捷徑**：9.01 的 `INSTALL.EXE` 是 MZ＋NE、目標作業系統是 OS/2，
DOS stub 只印 `SYS2090`，所以沒辦法用 dosgolem 跑原廠工具解自己的封包。
磁片上 183 個檔案裡 181 個是 WPK，另外兩個就是它與純文字的 `INSTALL.SCR`。

**順帶更正**：檔名長度的最高位元是 `NO_SHANNON_CODE`（不用 Shannon-Fano，**仍是 LZSS 壓縮**），
不是「未壓縮」；先前的盤點工具與筆記都讀錯了。

下一步兩條（都還沒做）：反組譯 6.5 或 7.0 函式庫裡的 `qsort` 看等值順序；
或反組譯 `INSTALL.EXE` 的 NE 段，那就是 1992 年的解包器本體。
經過與可重建的工作區記在私有工作區的 `tools/wpk/`。
