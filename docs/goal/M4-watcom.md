...
接 R53（私有 #32，`watcom65-wcg-exit1`）：`[0x50F4]＝0x498C` 是
`3956:1610` 存入（∈`sub_24CF1`），值出自 `26C2` 段 bump-allocator
（∈`sub_10E1C`）；上游 checker∈`sub_2CA9C`、caller∈`sub_2CDAF`
（`4207` 基三處一致）、wrapper 九處同體。步 51982 跳變記異常。
完成條件：五處身份＋鏈條；筆記更新。本輪不做：實作、WCC 側改動、7.0、對拍。

## R55（本輪）：根因＝spawn 壓活塊，spec-205 就緒

接 R54（私有 #32，`watcom65-wcg-exit1`）：分支盤點（744 筆／161 位址）＋
堆疊平衡（16k 步零洩漏）後收斂——EXEC 前零釋放，`spawn` 盲用 `freeSeg+1`
蓋住活 arena 塊（步 38489 實證）；修法寫成 dosgolem spec-205 READY
