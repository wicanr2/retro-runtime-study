...
| R21 | M1 第四批：iostream（#24）| 主文 `docs/10-borland-crtl/iostream.md`；C++ 產生碼指紋視份量獨立成 `bcc20-cxx-codegen.md`；`examples/iostream/`；私有 `topics/iostream.md`（#41） |
| R22 以後 | 依 worklist 排序 | Watcom 8.0 與 9.5、9.01 啟動多型（庫內 cstart 與 adsstart.obj 的分工） |
...
審查（R18.5）抓到的問題全部集中在**證據等級的一致性**，沒有事實錯誤：
「已證實（實測）」那句把 7.0 的 helper 統計歸給自寫的 OMF 解析器，但那一版的函式庫是
Easy OMF-386、解析器讀不動，實際用的是字串比對；指標 helper 整列合併打一個「架構推論」標籤，
蓋掉了其中兩個其實查過字串這件事；還有一句解包前的舊稿（「這個描述是從大小與檔名推的」）
殘留在已經改寫成讀原始碼的段落裡，與同段前半自相矛盾——**改寫既有段落時最容易漏掉的就是
這種「前半改了、後半沒改」**。另外補上了三處「知道所以沒解釋」的機制
（`BEGTEXT` 怎麼保證函式指標不為 NULL、`XIB`／`XI`／`XIE` 怎麼靠連結器拼表、
`'PHAR'` 那一手為什麼會有反應），並把兩處無證據的加碼收回
（`__StartTime`「改由別處處理」、stub「執行完就不存在」）。
