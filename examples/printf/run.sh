#!/bin/sh
# 在 dosgolem 裡用 Borland C++ 2.0 編譯 PRINTF.C 並執行，印出 OUT.TXT。
#
#   BCPP=~/bcpp20 DOSGOLEM=~/dosgolem ./run.sh [模型 s|c|m|l|h] [BCC 選項…]
#   例：./run.sh l -O -G -Z
#
# 產物：
#   out/printf.asm   BCC 以 -S 輸出的組語（編譯器對 PRINTF.C 產生的指令，含原始碼行註解）
#   out/printf.obj   不經組譯器、BCC 直接產生的目的檔（反組譯看位元組用這份）
...
rm -rf "$here/out"
"$bcc" "$here" BCC.EXE -m$model -S "$@" PRINTF.C > "$here/build.log"
mv "$here/out/printf.asm" "$here/printf.asm.tmp"
"$bcc" "$here" BCC.EXE -m$model "$@" PRINTF.C >> "$here/build.log"
mv "$here/printf.asm.tmp" "$here/out/printf.asm"
test -f "$here/out/printf.exe" || { cat "$here/build.log"; echo "沒有產出 out/printf.exe" >&2; exit 1; }

run=$here/out/run
mkdir -p "$run/root" "$run/scratch"
cp "$here/out/printf.exe" "$run/root/PRINTF.EXE"
timeout 10m docker run --rm --network none -u "$(id -u):$(id -g)" \
    ...
    retro-runtime-study-tools:1 /dosrun -prog /root/PRINTF.EXE -root /root -scratch /out \
