#!/bin/sh
# 在 dosgolem 裡用 Borland C++ 2.0 編譯 ORDER.C 並執行，印出 ORDER.TXT。
#
#   BCPP=~/bcpp20 DOSGOLEM=~/dosgolem ./run.sh [模型 s|c|m|l|h]
#
# 預設 small。輸出一行字元，依序代表各函式被呼叫的順序（見 README）。
# 結果與 expected.txt 比對。
set -eu
...
rm -rf "$here/out"
"$here/../../tools/bcpp20/bcc.sh" "$here" BCC.EXE -m$model ORDER.C > "$here/build.log"
test -f "$here/out/order.exe" || { cat "$here/build.log"; echo "沒有產出 out/order.exe" >&2; exit 1; }

run=$here/out/run
mkdir -p "$run/root" "$run/scratch"
cp "$here/out/order.exe" "$run/root/ORDER.EXE"
timeout 10m docker run --rm --network none -u "$(id -u):$(id -g)" \
    ...
    retro-runtime-study-tools:1 /dosrun -prog /root/ORDER.EXE -root /root -scratch /out \
