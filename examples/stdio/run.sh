#!/bin/sh
# 在 dosgolem 裡用 Borland C++ 2.0 編譯 STDIO.C 並執行，印出 OUT.TXT。
#
#   BCPP=~/bcpp20 DOSGOLEM=~/dosgolem ./run.sh [模型 s|c|m|l|h] [BCC 選項…]
#   例：./run.sh l -O -G -Z
#
# 產物：
#   out/stdio.asm   BCC 以 -S 輸出的組語（編譯器對 STDIO.C 產生的指令，含原始碼行註解）
#   out/stdio.obj   不經組譯器、BCC 直接產生的目的檔（反組譯看位元組用這份）
...
rm -rf "$here/out"
"$bcc" "$here" BCC.EXE -m$model -S "$@" STDIO.C > "$here/build.log"
mv "$here/out/stdio.asm" "$here/stdio.asm.tmp"
"$bcc" "$here" BCC.EXE -m$model "$@" STDIO.C >> "$here/build.log"
mv "$here/stdio.asm.tmp" "$here/out/stdio.asm"
test -f "$here/out/stdio.exe" || { cat "$here/build.log"; echo "沒有產出 out/stdio.exe" >&2; exit 1; }

run=$here/out/run
mkdir -p "$run/root" "$run/scratch"
cp "$here/out/stdio.exe" "$run/root/CODEGEN.EXE"
