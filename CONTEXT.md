...
| LCG（線性同餘產生器） | 偽亂數演算法：狀態 × 乘數 ＋ 增量。Borland 的 `rand` 用 32 位元狀態、乘數 0x015A4E35、增量 1，輸出取狀態第 16–30 位元 |
...
| 尾端底線修飾 | Watcom 暫存器呼叫慣例的函式符號形態（`strlen_`）；前置兩個底線的則是啟動碼設的全域變數與 helper |
| `CLIBM_TEXT` 這類段名 | Watcom 函式庫的程式碼段用「庫名＋`_TEXT`」，段名直接說出連的是哪個模型的庫 |
| `#pragma aux` | Watcom 用來逐函式指定呼叫慣例的指示：哪些暫存器傳參、哪些會被改動、參數順序 |
...
| stub | 「16 位元前導程式 ＋ 32 位元本體」黏成一個執行檔時前面那一段，負責找到 extender 再交出控制權。Watcom 的版本（`wstub.c`）不是載入器，它找到 `dos4gw.exe` 就用 DOS 的 EXEC 把自己換掉 |
...
| Easy OMF-386 | Phar Lap 的 32 位元 OMF 變體：模組開頭一筆 COMENT（class `0xAA`）帶 `80386` 標記，之後段長與位移類欄位是 4 位元組；外層 `.LIB` 封裝與標準 OMF library 相同。Watcom 7.0 與 9.01 的出貨函式庫用的是這種 |
